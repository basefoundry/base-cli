...
class AppSubcommandTests(unittest.TestCase):
    ...
    def test_runs_multiple_subcommands_with_base_lifecycle(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(seen["hello"]["temp_dir"].is_dir())
            self.assertTrue(seen["clean"]["temp_dir"].is_dir())
            self.assertEqual(list(seen["hello"]["temp_dir"].iterdir()), [])
            self.assertEqual(list(seen["clean"]["temp_dir"].iterdir()), [])
